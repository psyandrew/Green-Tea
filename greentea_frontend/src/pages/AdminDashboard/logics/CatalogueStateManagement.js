...
export const useInventoryRequests = () => {
  ...
  useEffect(() => {
    const fetchInventory = async() => {
      ...
      try { 
        ...
        const response = await fetch('https://green-tea-production.up.railway.app/admin/products/all')
  ...
  const updateRequestInventory = async ( type, productID, data ) => {
    ...
    const requestMap = {
      "inventory": {
        "fetchString":'http://127.0.0.1:8000/admin/products/update/stock',
        "requestKey": 'inventory_stock'
      },
      "price": {
        "fetchString":'http://127.0.0.1:8000/admin/products/update/price',
        "requestKey": 'price'
      },
      "on_sale": {
        "fetchString":'http://127.0.0.1:8000/admin/products/update/status',
        "requestKey": 'on_sale'
      }
    }
